...
SELECT year,count(title) from movie group by year order by year asc;
-- Number of movies released each month.
SELECT month(date_published) as month_num,count(title) as number_of_movies from movie group by month(date_published) order by month(date_published) asc;
...
SELECT genre,count(title) as no_of_movies from genre g inner join movie m on g.movie_id = m.id group by genre order by no_of_movies desc;
...
SELECT genre,avg(duration) as avg_duration from genre g inner join movie m on g.movie_id = m.id group by genre;
